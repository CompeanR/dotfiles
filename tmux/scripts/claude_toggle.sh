...
if ! tmux -L "$POPUP_SOCKET" has-session -t "$POPUP_SESSION" 2>/dev/null; then
    if [[ "$OSTYPE" == "darwin"* ]]; then
        tmux -L "$POPUP_SOCKET" -f /dev/null new-session -d -s "$POPUP_SESSION" -c "$CURRENT_DIR" "cd '$CURRENT_DIR' && while true; do cyc || /opt/homebrew/bin/claude; done"
    else
        tmux -L "$POPUP_SOCKET" -f /dev/null new-session -d -s "$POPUP_SESSION" -c "$CURRENT_DIR" "cd '$CURRENT_DIR' && while true; do /home/compean/.nvm/versions/node/v22.17.1/bin/claude --dangerously-skip-permissions; done"
    fi
...
"$POPUP_REBIND_SCRIPT" "$POPUP_SOCKET" "$PARENT_NAV_SCRIPT"

...
# Lazygit popup toggle using session name detection (window-specific)
TOOL="lazygit"
...
if ! tmux -L "$POPUP_SOCKET" has-session -t "$POPUP_SESSION" 2>/dev/null; then
    tmux -L "$POPUP_SOCKET" -f /dev/null new-session -d -s "$POPUP_SESSION" -c "$CURRENT_DIR" "cd '$CURRENT_DIR' && while true; do lazygit; done"
...
tmux -L "$POPUP_SOCKET" unbind -n M-{ 2>/dev/null
tmux -L "$POPUP_SOCKET" unbind -n M-} 2>/dev/null
tmux -L "$POPUP_SOCKET" unbind -n q 2>/dev/null
...
tmux -L "$POPUP_SOCKET" bind-key -n q run-shell -b "$POPUP_CLOSE_SCRIPT"

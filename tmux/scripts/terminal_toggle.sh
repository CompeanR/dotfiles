...
if ! tmux -L "$POPUP_SOCKET" has-session -t "$POPUP_SESSION" 2>/dev/null; then
    tmux -L "$POPUP_SOCKET" -f /dev/null new-session "${POPUP_ENV_ARGS[@]}" -d -s "$POPUP_SESSION" -c "$CURRENT_DIR"
...
"$POPUP_REBIND_SCRIPT" "$POPUP_SOCKET" "$PARENT_NAV_SCRIPT" --bind-format prefix C-p
